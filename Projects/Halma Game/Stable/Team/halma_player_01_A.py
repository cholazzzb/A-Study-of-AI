...
class HalmaPlayer01A:

    nama = "BarbarA"
    deskripsi = "Barbar StrategyA"
    #nomor = 1
    #index = 0
    papan = []
    teman = None
    ...
    def main(self, model):
        ...
        if(mem.firstrun == True):
            mem.goall=player_goals(self,self.index)
            mem.firstrun=False
        
        print(mem.pmA)
        memory=5
        if len(mem.pmA)==memory:
            mem.pmA.pop(0)
        ...
        def eval_func(self, index, b, gl):
            ...
            distances = [dist(b, goal) for goal in mem.goall]
            #print(distances)
            for d in distances:
